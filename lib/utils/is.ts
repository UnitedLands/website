...
module is {
	export function aString(what: any): what is string {
		return isString(what)
	}

	export function notaString<T>(what: T): what is Exclude<T, string> {
		return !aString(what)
	}

	export function aObject<Thing extends Record<any, any>>(what: any): what is Thing {
		return isObject(what)
	}

	export function notaObject<T, O extends Record<any, any>>(what: T): what is Exclude<T, O> {
		return !aObject(what)
	}
	...
	export function notaArray<T>(what: T): what is Exclude<T, any[]> {
		return !aArray<any[]>(what)
	}

	export function aFunction<Thing extends (...args: any[]) => any>(what: any): what is Thing {
		return isFunction(what)
	}

	export function notaFunction<T>(what: T): what is Exclude<T, (...args: any[]) => any | Function> {
