...
export class Mailer {
	private declare _mail: Mail
	from: string = process.env.SMTP_FROM ?? 'app@construct'

	constructor(args: SMTPTransport.Options) {
		if (!args.host) {
			console.info('missing smtp host, skipping verification')
			return
		}
	...
	send(args: Mail.Options) {
		if (!this._mail) {
			console.info('mailer not setup, skipping send and resolving')
